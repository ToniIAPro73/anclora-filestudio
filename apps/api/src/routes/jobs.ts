...
import { defaultAgentService, getProjectedAgentJob } from "./agent.js";
...
export function createJobsRouter(): Hono {
  ...
  app.post("/", requireScope("filestudio:jobs:create"), async (c) => {
    ...
    if (!jobService) return c.json({ type: "about:blank", title: "Service Unavailable", status: 503, code: "ENGINE_UNAVAILABLE" }, 503);
  ...
  app.get("/:id", requireScope("filestudio:jobs:read"), async (c) => {
    const auth = c.get("auth") as AuthContext;
    const jobService = c.get("jobService") as JobService | undefined;
    const agentJob = getProjectedAgentJob(defaultAgentService, c.req.param("id")!, auth);
    if (agentJob) return c.json(toPublicJob(agentJob));
