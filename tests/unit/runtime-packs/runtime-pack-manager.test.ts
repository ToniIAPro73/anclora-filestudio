...
async function createFakePack(entries: Record<string, string>): Promise<{ archivePath: string; sha256: string; size: number }> {
  const root = tempDir("runtime-pack-fixture");
  const zip = new JSZip();
  for (const [name, content] of Object.entries(entries)) {
    zip.file(name, content, { unixPermissions: name.endsWith("fake-runtime") ? 0o100755 : 0o100644 });
  }
...
function definition(overrides: Partial<RuntimePackDefinition> = {}): RuntimePackDefinition {
  return {
    ...
    executablePaths: {
      linux: "bin/fake-runtime",
      windows: "bin/fake-runtime.exe",
...
describe("RuntimePackManager", () => {
  ...
  it("PACK-004 rejects SHA mismatch and leaves no executable", async () => {
    const pack = await createFakePack({ "bin/fake-runtime": "#!/bin/sh\necho Fake 1.0.0\n" });
    ...
    expect(fs.existsSync(path.join(root, "fake-runtime", "1.0.0", "bin", "fake-runtime"))).toBe(false);
  });
