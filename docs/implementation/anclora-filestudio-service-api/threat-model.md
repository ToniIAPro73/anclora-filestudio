...
### A-19: Consumidor multi-app externo

| Campo | Valor |
|---|---|
| Actor | App consumidora comprometida o mal configurada |
| Vector | Token válido de `client_id` externo crea jobs excesivos o intenta leer jobs de otro workspace |
| Impacto | DoS, fuga cross-tenant, coste operativo |
| Mitigación | Scope dedicado `filestudio:agent-jobs:create`; filtros por `client_id` y `workspaceId`; límites por scope/client en rate-limit; tamaño máximo por operación; cola con expiración si el Local Agent está apagado |
| Test | `apps/api/tests/agent.test.ts` |
| Riesgo residual | Medio hasta mover `AgentJobRecord` de in-memory a persistencia PostgreSQL |
