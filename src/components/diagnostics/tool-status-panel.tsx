...
import { CheckCircle2, XCircle, Info, Loader2, RefreshCw, AlertTriangle, Terminal } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Dependency Setup Wizard for Linux and Windows */}
          <div className="rounded-[18px] border border-cyan-500/20 bg-[#161a20] p-4 text-sm text-stone-200 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-cyan-400 font-semibold text-xs tracking-wider uppercase">
                <Terminal className="h-4 w-4" />
                <span>Asistente de Instalación y Dependencias (Linux & Windows)</span>
              </div>
              <span className="text-[10px] text-cyan-300/70 bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 rounded-full">
                Auto-Diagnóstico Pro
              </span>
            </div>

            <p className="text-xs text-stone-400 leading-relaxed">
              Si ejecutas Anclora FileStudio en Linux o Windows Desktop PRO, puedes instalar o actualizar todos los motores de conversión con los siguientes comandos oficiales:
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
              {/* Linux tab */}
              <div className="rounded-xl border border-white/8 bg-black/40 p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-emerald-400">🐧 Ubuntu / Debian / Linux</span>
                  <button
                    type="button"
                    onClick={() => {
                      navigator.clipboard.writeText("sudo apt update && sudo apt install -y ffmpeg pandoc qpdf tesseract-ocr poppler-utils p7zip-full libreoffice calibre");
                    }}
                    className="text-[10px] bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-1 rounded-md transition-colors"
                  >
                    Copiar comando
                  </button>
                </div>
                <code className="block font-mono text-[11px] text-stone-300 bg-black/60 p-2 rounded-lg overflow-x-auto select-all">
                  sudo apt update && sudo apt install -y ffmpeg pandoc qpdf tesseract-ocr poppler-utils p7zip-full libreoffice calibre
                </code>
                <p className="text-[10px] text-stone-500">
                  O ejecuta directamente el script incluido: <span className="text-emerald-400/90 font-mono">./scripts/setup-ubuntu.sh</span>
                </p>
              </div>
              ...
              <div className="rounded-xl border border-white/8 bg-black/40 p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-sky-400">🪟 Windows (PowerShell / winget)</span>
                  <button
                    type="button"
                    onClick={() => {
                      navigator.clipboard.writeText("winget install Gyan.FFmpeg JohnMacFarlane.Pandoc QPDF.QPDF UB-Mannheim.TesseractOCR 7zip.7zip");
                    }}
                    className="text-[10px] bg-sky-500/10 hover:bg-sky-500/20 text-sky-300 border border-sky-500/30 px-2 py-1 rounded-md transition-colors"
                  >
                    Copiar comando
                  </button>
                </div>
                <code className="block font-mono text-[11px] text-stone-300 bg-black/60 p-2 rounded-lg overflow-x-auto select-all">
                  winget install Gyan.FFmpeg JohnMacFarlane.Pandoc QPDF.QPDF UB-Mannheim.TesseractOCR 7zip.7zip
                </code>
                <p className="text-[10px] text-stone-500">
                  O ejecuta en el ejecutable portable: <span className="text-sky-400/90 font-mono">ACTUALIZAR_YTDLP.bat</span>
                </p>
              </div>
            </div>
          </div>
