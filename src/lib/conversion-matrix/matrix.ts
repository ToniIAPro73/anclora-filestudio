...
const QUALITY_OVERRIDES: Record<string, EdgeQualityInput> = {
  ...
  "png->gif": { preservation: { mediaQuality: 0.55, alpha: 0 }, irreversibleLosses: ["mediaQuality", "alpha"], reencodeRequired: true },
  "jpg->gif": { preservation: { mediaQuality: 0.55, alpha: 0 }, irreversibleLosses: ["mediaQuality", "alpha"], reencodeRequired: true },
  "webp->gif": { preservation: { mediaQuality: 0.55, alpha: 0 }, irreversibleLosses: ["mediaQuality", "alpha"], reencodeRequired: true },
  "tiff->gif": { preservation: { mediaQuality: 0.55, alpha: 0 }, irreversibleLosses: ["mediaQuality", "alpha"], reencodeRequired: true },
  "avif->gif": { preservation: { mediaQuality: 0.55, alpha: 0 }, irreversibleLosses: ["mediaQuality", "alpha"], reencodeRequired: true },
...
export const CANONICAL_CONVERSION_EDGES: readonly CanonicalConversionEdge[] = [
  ...
  edge({
    source: "pdf",
    target: "odt",
    operationId: "office:pdf-to-odt",
    implementationId: "libreoffice-pdf-import-odt",
    engineId: "libreoffice",
    dependencies: ["libreoffice", "pdftotext"],
    lossProfile: "structural-risk",
    priority: 88,
    supportsAsIntermediate: false,
    quality: { preservation: { text: 0.9, structure: 0.58, layout: 0.55, tables: 0.35, images: 0.85, metadata: 0.5 }, irreversibleLosses: ["tables", "structure"], reencodeRequired: false, runtimeCost: "high", stability: 0.88, certification: "benchmarked" },
    contentRequirements: { requiresTextLayer: true },
    notes: "LibreOffice writer_pdf_import rebuilds an editable ODT directly from PDF. Quality matches the certified PDF→DOCX family: text/unicode/multipage are useful; tables and semantic styles are degraded. Scanned PDFs are rejected with a controlled OCR hint. Not an intermediate.",
  }),
